# Accessibility Auditor — Wyndham (WCAG 2.2 AA)
# One-file Streamlit app with: card UI, top nav anchors, light/dark mode, crawler modal,
# contrast + alt checks, PDF tagging heuristics, Wyndham-branded PDF export, and history dashboard.
...
.sidebar .block-container {{ padding-top: 8px !important; }}
</style>
...
def _hex_to_rgb(hex_color: str) -> Tuple[int,int,int]:
    hex_color = _norm_hex(hex_color)
    if not hex_color.startswith('#'): return (0,0,0)
    return (int(hex_color[1:3],16), int(hex_color[3:5],16), int(hex_color[5:7],16))

def _parse_css_color(val: Optional[str]) -> Tuple[int,int,int]:
    if not val: return (0,0,0)
    v = val.strip()
    if HEX_RE.match(v): return _hex_to_rgb(v)
    m = RGBA_RE.match(v)
    if m:
...
# HTML analyzer (quick-check, inline styles only)
...
def analyze_html(url: str) -> Dict:
    ...
    for el in soup.find_all(TEXT_TAGS):
        ...
        if _passes_aa(cr,size_px,bold):
            ...
            contrast_issues.append(ContrastIssue(tag=el.name, text=text, fg=f"#{fg[0]:02x}{fg[1]:02x}{fg[2]:02x}", bg=f"#{bg[0]:02x}{bg[1]:02x}{bg[2]:02x}", ratio=round(cr,2), size_px=size_px, bold=bold))
    ...
    for img in soup.find_all("img"):
        ...
        if alt == "":
            ...
            img_issues.append(ImgAltIssue(src=src, suggestion=f"Add meaningful alt text, e.g. alt=\"Wyndham City Council logo\""))
...
"
        f"• Darken text (e.g., color:#111) or lighten background (e.g., #FFF).
"
...
def build_pdf_report(path: str, brand: Dict, target_url: str, html_result: Optional[Dict], pdf_results: List[PdfAccessibility]):
    if HAVE_REPORTLAB:
        ...
        story = []
        try:
            logo = ImageReader(requests.get(brand["logo"], timeout=10).content)
            story.append(Spacer(1,4)); story.append(Paragraph(f"<b>{brand['name']}</b>", styles['Heading1']))
        except Exception:
        ...
        if pdf_results:
            ...
            t2 = Table(rows, colWidths=[180,40,50,50,70,120]); t2.setStyle(TableStyle([('INNERGRID',(0,0),(-1,-1),0.25,colors.grey), ('BOX',(0,0),(-1,-1),0.25,colors.grey)]))
            story.append(t2)
        doc.build(story); return
    if HAVE_FPDF:
        pdf = FPDF(); pdf.set_auto_page_break(auto=True, margin=15); pdf.add_page(); pdf.set_font("Arial","B",16)
        pdf.cell(0,10,txt=f"{brand['name']} — Accessibility Audit", ln=True)
        pdf.set_font("Arial", size=12); pdf.cell(0,8,txt=f"Target URL: {target_url}", ln=True); pdf.cell(0,8,txt=datetime.datetime.now().strftime("Generated: %Y-%m-%d %H:%M"), ln=True)
        pdf.ln(3)
        if html_result:
...
# =====================
# Main: Cards — Run Audit / Export Report
# =====================
...
with col1:
    ...
    if run_pdfs:
        ...
        for line in (pdf_urls_text or "").splitlines():
            u = line.strip(); if not u: continue
            try:
            ...
            if not (html_result or pdf_results):
                ...
                try:
                    ...
                    with open(out_path, "rb") as f:
                        st.download_button("⬇️ Download Report PDF", data=f.read(), file_name=os.path.basename(out_path), mime="application/pdf")
                except Exception as e:
    ...
    with st.expander("Contrast fails (examples)"):
        if html_result["contrast_issues"]:
            df_ci = pd.DataFrame(html_result["contrast_issues"])\
                .drop(columns=["selector_hint"], errors="ignore")
...
st.caption("Notes: Contrast quick‑check uses inline styles only. Full audit requires computed styles, keyboard focus, ARIA checks.")
